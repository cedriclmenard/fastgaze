...
class BlazeFace(nn.Module):
    ...
    """
    default_weights_path = str(Path(os.path.dirname(__file__), "blazeface.pth"))
    default_anchors_path = str(Path(os.path.dirname(__file__), "anchors.npy"))
    def __init__(self, min_score=0.75):
        ...
        self.min_score_thresh = min_score
    ...
    def predict_on_batch(self, x, only_best=False):
        ...
        for i in range(len(detections)):
            ...
            faces = torch.stack(faces) if len(faces) > 0 else torch.zeros((1, 17), device=self._device())
            if only_best:
                filtered_detections.append(faces[0])
            else:
