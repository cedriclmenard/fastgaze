...
class FaceMesh(nn.Module):
    ...
    """
    default_weights_path = str(Path(os.path.dirname(__file__), "facemesh.pth"))
    def __init__(self):
    ...
    def forward(self, x):
        ...
        # x = nn.ConstantPad2d((1, 0, 1, 0), 0.0)(x)
        x = F.pad(x, (1, 0, 1, 0), "constant", 0.0)
    ...
    def predict_on_image(self, img, output_confidence=False):
        ...
        dets, conf = self.predict_on_batch(img.unsqueeze(0))
        if output_confidence:
            return dets.squeeze(), conf.squeeze()
        else:
            return dets
    ...
    def predict_on_batch(self, x):
        ...
        # detections = detections.view(-1, 468, 3)
        # detections[:,:,0] *= self.x_scale
        # detections[:,:,1] *= self.y_scale

        return detections.view(-1, 468, 3), confidences
